...
def standartization(comp):
    ...
    newcomp = DataFrame(columns = cols)
    ...
    for j in comp.index:
        if j > days_for_normalization - 1:
            ...
            if sample_variance == 0:
                ...
                row.append((comp.iloc[j, 5] - average_volume) / (4 * sqrt(sample_variance)))
            ...
            for i in sets:
                ...
                if sample_variance == 0:
                    ...
                    row.append((i.iloc[days_for_normalization] - average_price) / (4 * sqrt(sample_variance)))
                row.append(sample_variance)
            ...
            for i in sets:
                ...
                if sample_variance == 0:
                    ...
                    row.append((prices.iloc[days_for_normalization-1] - average_price) / (4 * sqrt(sample_variance)))
                row.append(sample_variance)
...
def detectionResults(symbol, startDate, endDate, to_include_KNN, to_include_GBT, path):
    ...
    try:
        ...
        if data.isnull().any().any() or (inf in data.values) or (-inf in data.values) or (0.0 in data.values):
    ...
    try:
        ...
        return 'Yahoo! Finance returned unexpected columns'
    
    try:
        if to_include_KNN == True:
            KNN = load(open('KNNModel.sav', 'rb'))
            KNN_results = DataFrame(KNN.predict(standarised_data), columns = ['KNN'])
            results = concat([results, KNN_results], axis=1)
    except:
        return 'Failed to load KNN results'
    
    try:
        if to_include_GBT == True:
            GBT = load(open('GBTModel.sav', 'rb'))
            GBT_results = DataFrame(GBT.predict(standarised_data), columns = ['GBT'])
            results = concat([results, GBT_results], axis=1)
    except:
    ...
    try: 
        ...
        return 'Failed to save an excel file'
        ...
        if to_include_KNN == False and to_include_GBT == False:
            ...
            if symbol == '':
                ...
                if start > end:
                    ...
                    errorLine = detectionResults(symbol, start, end, to_include_KNN, to_include_GBT, self.OutputPathLine.text())
        
        self.ErrorLine.setText(errorLine)
        
    def choose(self):
        directory = QFileDialog.getExistingDirectory(self, "Choose the output path", self.OutputPathLine.text())
